...
.. _integrator-ptracerfluoro:

Particle tracer with fluorescence (:monosp:`ptracerfluoro`)
-----------------------------------
...
This integrator is based on the particle tracer, but supports fluorescent
(wavelength-shifting) BRDFs.

Compared to the particle tracer, at every intersection, this integrator
...
ray, with the new wavelength sampled from the material's emission spectrum.
...
class ParticleTracerFluoroIntegrator final : public AdjointIntegrator<Float, Spectrum> {
public:
    ...
    ParticleTracerFluoroIntegrator(const Properties &props) : Base(props) { }
    ...
    void sample_visible_emitters(const Scene *scene, const Sensor *sensor,
        ...
        connect_sensor(scene, si, sensor_ds, nullptr, weight, false, si, block, sample_scale, active);
    }
    ...
    trace_light_ray(Ray3f ray, const Scene *scene, const Sensor *sensor,
        ...
        dr::tie(ls) = dr::while_loop(dr::make_tuple(ls),
            ...
            /* If a fluorescent component is sampled, sample the emission
               distribution to shift the wavelength of the outgoing ray. The
               new wavelength is used when splatting to the sensor and
               generating the next ray, but the original wavelength is used
               evaluate the fluorescent excitation of the hit object itself. */
            Mask is_fluoro = has_flag(bs.sampled_type,
                                      BSDFFlags::FluorescentReflection);
            SurfaceInteraction3f shifted_si = si;
            UnpolarizedSpectrum emission_weight (1.f);
            if (dr::any_or<true>(is_fluoro)) {
                auto [fluoro_wavelengths, fluoro_weight] =
                    bsdf->sample_wavelength_shift(ctx, si, ls.sampler->next_1d(ls.active));
                dr::masked(shifted_si.wavelengths, is_fluoro) = fluoro_wavelengths;
                dr::masked(emission_weight,        is_fluoro) = fluoro_weight;
            }
            ...
            connect_sensor(scene, si, sensor_ds, bsdf,
                           ls.throughput * sensor_weight * emission_weight,
                           is_fluoro, shifted_si, block, sample_scale,
                           ls.active);
            ...
            ls.ray = shifted_si.spawn_ray(si.to_world(bs.wo));
        ...
        "Particle Tracer Fluoro Integrator");
    ...
    Spectrum connect_sensor(const Scene *scene, const SurfaceInteraction3f &si,
                            ...
                            const Mask &is_fluoro, const SurfaceInteraction3f &shifted_si,
        ...
        if (dr::any_or<true>(on_surface)) {
            ...
            if (dr::any_or<true>(on_surface)) {
                ...
                Spectrum bsdf_eval = dr::select(is_fluoro,
                    bsdf->eval_fluoro(ctx, si, local_d, on_surface),
                    bsdf->eval(ctx, si, local_d, on_surface));

                surface_weight[on_surface] *= correction * bsdf_eval;
            }
        }
        ...
        Wavelength output_wavelengths = dr::select(
            is_fluoro, shifted_si.wavelengths, si.wavelengths);
        block->put(adjusted_position, output_wavelengths, result, alpha,
                   /* weight = */ 0.f, active);
    ...
    std::string to_string() const override {
        return tfm::format("ParticleTracerFluoroIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "]",
    ...
    MI_DECLARE_CLASS(ParticleTracerFluoroIntegrator)
};

MI_EXPORT_PLUGIN(ParticleTracerFluoroIntegrator)
