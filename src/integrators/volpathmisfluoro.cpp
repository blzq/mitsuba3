...
class VolpathMisFluoroIntegratorImpl;

/**!

.. _integrator-volpathmisfluoro:

Volumetric path tracer with spectral MIS and fluorescence (:monosp:`volpathmisfluoro`)
---------------------------------------------------------------
...
class VolumetricMisFluoroPathIntegrator final : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    VolumetricMisFluoroPathIntegrator(const Properties &props) : Base(props) {
        m_use_spectral_mis = props.get<bool>("use_spectral_mis", true);
        m_props = props;
    }

    template <bool SpectralMis>
    using Impl = VolpathMisFluoroIntegratorImpl<Float, Spectrum, SpectralMis>;
    ...
    MI_DECLARE_CLASS(VolumetricMisFluoroPathIntegrator)
...
template <typename Float, typename Spectrum, bool SpectralMis>
class VolpathMisFluoroIntegratorImpl final : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    VolpathMisFluoroIntegratorImpl(const Properties &props) : Base(props) {}
    ...
    std::string to_string() const override {
        return tfm::format("VolumetricMisFluoroPathIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "]",
    ...
    MI_DECLARE_CLASS(VolpathMisFluoroIntegratorImpl)
};

MI_EXPORT_PLUGIN(VolumetricMisFluoroPathIntegrator)

NAMESPACE_BEGIN(detail)
template <bool SpectralMis>
constexpr const char * volpath_class_name() {
    if constexpr (SpectralMis) {
        return "Volpath_spectral_mis_fluoro";
    } else {
        return "Volpath_no_spectral_mis_fluoro";
    }
}
