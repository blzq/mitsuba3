...
class PRBVolpathFluoroIntegrator(RBIntegrator):
    r"""
    .. _integrator-prbvolpathfluoro:

    Path Replay Backpropagation Volumetric Integrator with Fluorescence (:monosp:`prbvolpathfluoro`)
    -------------------------------------------------------------------------
    ...
    def sample(self,
        ...
        if mode == dr.ADMode.Forward:
            raise RuntimeError("PRBVolpathFluoroIntegrator doesn't support "
        ...
        while dr.hint(active,
            ...
            with dr.resume_grad(when=not is_primal):
                ...
                # Move bsdf.sample() to before emitter sampling to decide if
                # the interaction should be wavelength-shifting (fluorescent)
                with dr.suspend_grad():
                    ...
                    shifted_si.wavelengths[is_fluoro_surf] = surf_fluoro_wavelengths
                    excite_weight[is_fluoro_surf] = surf_fluoro_weight
                ...
                if dr.hint(self.use_nee, mode='scalar'):
                    ...
                    emitted, ds = self.sample_emitter(mei, shifted_si, active_e_medium, active_e_surface,
                    ...
                    if dr.hint(not is_primal, mode='scalar'):
                        self.sample_emitter(mei, shifted_si, active_e_medium, active_e_surface,
                ...
                # ------------------------ BSDF sampling -----------------------
                bsdf_eval = dr.select(is_fluoro_surf,
                                      excite_weight * bsdf.eval_fluoro(ctx, si, bs.wo, active_surface),
                                      bsdf.eval(ctx, si, bs.wo, active_surface))
                ...
                bsdf_ray = shifted_si.spawn_ray(si.to_world(bs.wo))
...
mi.register_integrator("prbvolpathfluoro", lambda props: PRBVolpathFluoroIntegrator(props))
